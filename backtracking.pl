...
certainHuman(S) :- ballPlayer(_,_,S), w_Human(S), checkField(S).
...
rankFields([H|T],S,RL) :-
    ...
    ((certainHuman(S), canThrowBall(S), throwBallEast(_,S)) -> ThrowEastPoints = 1 ; ThrowEastPoints = 0 ),
    ((certainHuman(S), canThrowBall(S), throwBallWest(_,S)) -> ThrowWestPoints = 1 ; ThrowWestPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallSouth(_,S)) -> ThrowSouthPoints = 1 ; ThrowSouthPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallNorth(_,S)) -> ThrowNorthPoints = 1 ; ThrowNorthPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallSouthEast(_,S)) -> ThrowSouthEastPoints = 1 ; ThrowSouthEastPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallNorthEast(_,S)) -> ThrowNorthEastPoints = 1 ; ThrowNorthEastPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallSouthWest(_,S)) -> ThrowSouthWestPoints = 1 ; ThrowSouthWestPoints = 0),
    ((certainHuman(S), canThrowBall(S), throwBallNorthWest(_,S)) -> ThrowNorthWestPoints = 1 ; ThrowNorthWestPoints = 0),

    Total is NActions + DangerPoints + ThrowEastPoints + ThrowSouthPoints + 
              ThrowWestPoints + ThrowNorthPoints + ThrowNorthEastPoints + 
              ThrowNorthWestPoints + ThrowSouthEastPoints + ThrowSouthWestPoints,
    RR = rr(Total,H),
    add(RR,LT,RL).
...
runloop(T,S0) :-
    ...
    %Get action from backtracking (Strategy) in this situation
    backtracking(S0,H),
    ...
    %The following are needed to check if ballPlayer
...
% HELPERS
% These are the utility functions that are used inside the backtracking and 
